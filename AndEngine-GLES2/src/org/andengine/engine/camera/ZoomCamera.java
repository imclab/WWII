 ...
 * @since 15:48:11 - 24.06.2010 TODO min/max(X/Y) values could be cached and
 *        only updated once the zoomfactor/center changed.
 */
public class ZoomCamera extends BoundCamera {
	...
	// Constructors
	// ===========================================================
	public ZoomCamera(final float pX, final float pY, final float pWidth,
			final float pHeight) {
		super(pX, pY, pWidth, pHeight);
	}

	public ZoomCamera(final float pX, final float pY, final float pWidth,
			final float pHeight, float zoomMin, float zoomMax) {
		super(pX, pY, pWidth, pHeight);
	...
	protected void applySceneToCameraSceneOffset(
		...
		if (zoomFactor != 1) {
			...
			MathUtils.scaleAroundCenter(Camera.VERTICES_TMP, zoomFactor,
					zoomFactor, this.getCenterX(), this.getCenterY()); // TODO
																		// Use a
																		// Transformation
																		// object
																		// instead!?!
	...
	protected void applySceneToCameraSceneOffset(final float[] pSceneCoordinates) {
		final float zoomFactor = this.mZoomFactor;
		if (zoomFactor != 1) {
			MathUtils.scaleAroundCenter(pSceneCoordinates, zoomFactor,
					zoomFactor, this.getCenterX(), this.getCenterY()); // TODO
																		// Use a
																		// Transformation
																		// object
	...
	protected void unapplySceneToCameraSceneOffset(
		...
		if (zoomFactor != 1) {
			...
			MathUtils.revertScaleAroundCenter(Camera.VERTICES_TMP, zoomFactor,
					zoomFactor, this.getCenterX(), this.getCenterY()); // TODO
																		// Use a
																		// Transformation
																		// object
	...
	protected void unapplySceneToCameraSceneOffset(
		...
		final float zoomFactor = this.mZoomFactor;
		if (zoomFactor != 1) {
			MathUtils.revertScaleAroundCenter(pCameraSceneCoordinates,
					zoomFactor, zoomFactor, this.getCenterX(),
					this.getCenterY()); // TODO Use a Transformation object
										// instead!?!
		}
	}
