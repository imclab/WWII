...
public class InputManager implements IOnSceneTouchListener, IScrollDetectorListener, IPinchZoomDetectorListener {
	...
	public void giveOrderToUnit(float x, float y) {
		if (mGameActivity.battle.getPhase() == Phase.combat && selectedElement != null) {
			if (selectedElement.getGameElement() instanceof Unit) {
				...
				if (g != null && g.getGameElement() instanceof Unit && g != selectedElement && !((Unit) g.getGameElement()).isDead()
	...
	public void updateOrderLine(GameSprite gameSprite, float x, float y) {
		if (mGameActivity.battle.getPhase() == Phase.deployment) {
			...
			if (tmxtile != null && tmxtile.getTileColumn() >= mGameActivity.battle.getMe().getXPositionDeployment()
					&& tmxtile.getTileColumn() <= mGameActivity.battle.getMe().getXPositionDeployment() + GameUtils.DEPLOYMENT_ZONE_SIZE - 1) {
				gameSprite.setPosition(x - gameSprite.getWidth() / 2, y - gameSprite.getHeight() / 2);
				gameSprite.getGameElement().setTilePosition(mGameActivity.battle.getMap().getTiles()[tmxtile.getTileRow()][tmxtile.getTileColumn()]);
			}
		} else {
			// get distance
			...
			// during combat phase
			mGameActivity.orderLine.setPosition(gameSprite.getX() + gameSprite.getWidth() / 2, gameSprite.getY() + gameSprite.getHeight() / 2, x, y);
			GameSprite g = getElementAtCoordinates(x, y);
			if (g != null && g.isVisible() && g != gameSprite && g.getGameElement() instanceof Unit && !((Unit) g.getGameElement()).isDead()
					&& ((Unit) g.getGameElement()).getArmy() != ((Unit) selectedElement.getGameElement()).getArmy()) {
				// attack
				mGameActivity.orderLine.setColor(Color.RED);
				mGameActivity.crossHairLine.setColor(Color.RED);
				mGameActivity.crossHairLine.setPosition(x - mGameActivity.crossHairLine.getWidth() / 2, y - mGameActivity.crossHairLine.getHeight() / 2);
				...
				// move
				Log.d("terrain", mGameActivity.battle.getMap().getTiles()[mGameActivity.tmxLayer.getTMXTileAt(x, y).getTileRow()][mGameActivity.tmxLayer
						.getTMXTileAt(x, y).getTileColumn()].getTerrain() + "");
