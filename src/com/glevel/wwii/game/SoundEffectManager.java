...
public class SoundEffectManager {
    ...
    private void loadMfxFromAssets(String soundName) {
        if (mMfxMap.get(soundName) == null) {
            try {
                Sound mSound = SoundFactory.createSoundFromAsset(mEngine.getSoundManager(), mContext, soundName
                        + ".ogg");
    ...
    private float getVolumeFromSoundPosition(float x, float y, boolean isLeftEar) {
        float distance = MapLogic.getDistanceBetween(x, y, mCamera.getCenterX() + (isLeftEar ? -1 : 1) * EARS_DISTANCE,
                mCamera.getCenterY());
