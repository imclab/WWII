...
public class GameActivity extends LayoutGameActivity implements OnNewSpriteToDraw {
	...
	public void updateSelectedElementLayout(final GameSprite selectedElement) {
		runOnUiThread(new Runnable() {
			...
			public void run() {
				...
				// health
				((TextView) mSelectedUnitLayout.findViewById(R.id.unitName)).setTextColor(getResources().getColor(unit.getHealth().getColor()));
				...
				if (unit.getRank() == Rank.ally && o != null) {
					if (o instanceof FireOrder) {
						...
						crosshair.setPosition(f.getTarget().getSprite().getX() - crosshair.getWidth() / 2,
								f.getTarget().getSprite().getY() - crosshair.getHeight() / 2);
						crosshair.setVisible(true);
					} else if (o instanceof MoveOrder) {
						MoveOrder f = (MoveOrder) o;
						crosshair.setColor(Color.GREEN);
						crosshair.setPosition(f.getxDestination() - crosshair.getWidth() / 2, f.getyDestination() - crosshair.getHeight() / 2);
						crosshair.setVisible(true);
					} else {
						crosshair.setVisible(false);
					}
		...
		for (Player player : battle.getPlayers()) {
			for (Unit unit : player.getUnits()) {
				if (unit.getOrder() != null && unit.getOrder() instanceof MoveOrder) {
					...
					if (newTile.getTileX() != unit.getTilePosition().getTileX() || newTile.getTileY() != unit.getTilePosition().getTileY()) {
	...
	public void onCreateScene(OnCreateSceneCallback pOnCreateSceneCallback) throws Exception {
		...
		selectionCircle = new SelectionCircle(GraphicElementFactory.mGfxMap.get("selection.png"), getVertexBufferObjectManager());
		...
		crossHairLine = new Crosshair(GraphicElementFactory.mGfxMap.get("crosshair.png"), getVertexBufferObjectManager(), distanceText);
	...
	private void openGameMenu() {
		...
		mGameMenuDialog.findViewById(R.id.surrenderButton).setOnClickListener(new OnClickListener() {
			@Override
			public void onClick(View v) {
				Dialog confirmDialog = new CustomAlertDialog(GameActivity.this, R.style.Dialog, getString(R.string.confirm_surrender_message),
		...
		mGameMenuDialog.findViewById(R.id.exitButton).setOnClickListener(new OnClickListener() {
			...
			public void onClick(View v) {
				...
				finish();
				GoogleAnalyticsHelper.sendEvent(getApplicationContext(), EventCategory.ui_action, EventAction.button_press, "exit_game");
			}
		});
	...
	private void prepareDeploymentPhase() {
		...
		deploymentZone = new DeploymentZone(battle.getMe().getXPositionDeployment() * 32.0f, 0.0f,
				(battle.getMe().getXPositionDeployment() + GameUtils.DEPLOYMENT_ZONE_SIZE) * 32.0f, battle.getMap().getHeight() * 32.0f,
	...
	private void startGame() {
		if (mDeploymentStartTime > 0) {
			GoogleAnalyticsHelper.sendTiming(getApplicationContext(), TimingCategory.in_game, TimingName.deployment_time,
