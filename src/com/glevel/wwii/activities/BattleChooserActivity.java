...
public class BattleChooserActivity extends MyActivity implements OnPageChangeListener {
	...
	private OnClickListener onMapSelectedListener = new OnClickListener() {
		...
		public void onClick(View v) {
			if (!isMapClicked) {
				...
				intent.putExtra(ArmyBuilderActivity.EXTRA_ARMY, (mRadioGroupArmy.getCheckedRadioButtonId() == R.id.usa_army ? ArmiesData.USA.ordinal()
	...
	private void setupUI() {
		...
		android.widget.FrameLayout.LayoutParams layoutParams = new android.widget.FrameLayout.LayoutParams(screenSize.x + 100, screenSize.y + 100);
	...
	public void onPageScrolled(int position, float positionOffset, int positionOffsetPixels) {
		...
		if (direction >= 0 && direction < mMapsCarousel.getAdapter().getCount()) {
			paginationDots[position].setAlpha(ALPHA_ACTIVE_DOT - positionOffset * (ALPHA_ACTIVE_DOT - ALPHA_PASSIVE_DOT));
			paginationDots[direction].setAlpha(ALPHA_PASSIVE_DOT + positionOffset * (ALPHA_ACTIVE_DOT - ALPHA_PASSIVE_DOT));
		}
	}
