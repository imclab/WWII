...
public class UnitsArrayAdapter extends ArrayAdapter<Unit> {
	...
	public static class ViewHolder {
		...
		private ImageView unitExperience;
	...
	public View getView(int position, View convertView, ViewGroup parent) {
		...
		if (viewHolder == null) {
			...
			viewHolder.unitExperience = (ImageView) view.findViewById(R.id.unitExperience);
		...
		if (position < mUnits.size()) {
			...
			if (unit.getWeapons().size() > 1) {
				...
				viewHolder.unitSecondaryWeaponName.setCompoundDrawablesWithIntrinsicBounds(secondaryWeapon.getImage(), 0, 0, 0);
			...
			if (mIsMyArmy) {
				// frags
				viewHolder.unitPriceFrags.setText("" + unit.getFrags());
				viewHolder.unitPriceFrags.setTextColor(mContext.getResources().getColor(android.R.color.white));
				viewHolder.unitPriceFrags.setCompoundDrawablesWithIntrinsicBounds(0, 0, R.drawable.ic_frag, 0);
			} else {
				// price
				viewHolder.unitPriceFrags.setText(mContext.getString(R.string.points, unit.getRealSellPrice(mIsMyArmy)));
